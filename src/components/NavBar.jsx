...
const NavBar = React.memo(() => {
  ...
  useEffect(() => {
    ...
    Promise.all([import("gsap"), import("gsap/Draggable")]).then(
      ([{ gsap }, { Draggable }]) => {
      ...
      <div>
        ...
        <div className="logo-portfolio-container" ref={logoPortfolioRef}>
          ...
          <div className="portfolio-wrapper" ref={wrapperRef}>
            <p className="font-bold portfolio-text">Nitesh's Portfolio</p>
            {!isMobile && (
        ...
        <ul>
          {navIcons.map(({ id, img, type, action }) => (
            <li key={id} onClick={() => handleIconClick({ type, action })}>
              <img
                src={img}
                className={`icon-hover ${type ? "cursor-pointer" : ""} ${type == "music" && isPlaying ? "animate-spin" : ""}`}
                alt={`icon-${id}`}
              />
            </li>
          ))}
